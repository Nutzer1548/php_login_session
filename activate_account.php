...
function activate_account(){
	...
	$result=$sql->query('select t.id as t_id, t.user_id, t.verify_string, u.id as u_id, u.login_email from task_activate_user ast join users as u on t.user_id=u.id where t.verify_string="'.$sql->escape_string($key).'" and u.login_email="'.$sql->escape_string($email).'" and u.flags&0x01=0;');

	if(!is_object($result)){
		$ret['error']='Unexpected database result'
		return $ret;
	}

	if($result->num_row==0){
		$ret['error']='nothing to activate. possibillities: 1) already activated, 2) never registered';
		return $ret;
	}
